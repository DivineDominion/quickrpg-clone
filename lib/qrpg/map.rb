module QuickRPG
    ...
    class << self
      ...
      def load(wnd, filename, tileset_name)
        tileset = Gosu::Image.load_tiles(
          wnd, Common::tileset_file_path(tileset_name + ".png"), 
          Common::TILE_SIZE, Common::TILE_SIZE, true)
      ...
      def initialize_map(wnd, filename, tileset)
        map_path = Common::map_file_path(filename + ".map")
        kol_path = Common::map_file_path(filename + ".kol")
        lyr_path = Common::map_file_path(filename + ".lyr")
      ...
      def initialize_scripts(wnd, filename, map)
        ani_path = Common::script_file_path(filename + ".ani") # TODO move somewhere else 2013-07-14
        scr_path = Common::script_file_path(filename + ".sc")
  ...
  public
    ...
    def scrolled_tile_x
      scrolled_x / Common::TILE_SIZE
    end
  
    def scrolled_tile_screen_width
      scrolled_tile_x + Common::SCREEN_WIDTH_TILE
    end
  
    def scrolled_tile_y
      scrolled_y / Common::TILE_SIZE
    end
  
    def scrolled_tile_screen_height
      scrolled_tile_y + Common::SCREEN_HEIGHT_TILE
    end
  ...
  protected
    ...
    def scrolling_possible?
      case @direction
      when :up
        @scrolled_y > 1 * Common::TILE_SIZE
      when :down
        (@scrolled_y + Common::SCREEN_HEIGHT) < (@height * Common::TILE_SIZE)
      when :left
        @scrolled_x > 1 * Common::TILE_SIZE
      when :right
        (@scrolled_x + Common::SCREEN_WIDTH) < (@width * Common::TILE_SIZE) 
      end
    end
    ...
    def center_map(x, y)
      unless @width <= Common::SCREEN_WIDTH_TILE
        swt = Common::SCREEN_WIDTH_TILE / 2
      ...
      unless @height <= Common::SCREEN_HEIGHT_TILE
        sht = Common::SCREEN_HEIGHT_TILE / 2
      ...
      @scrolled_x = x * Common::TILE_SIZE
      @scrolled_y = y * Common::TILE_SIZE
    end
    ...
    def draw_tiles
      ...
      to_x = from_x + Common::SCREEN_WIDTH_TILE
      to_x = to_x >= @width ? @width - 1 : to_x
    
      to_y = from_y + Common::SCREEN_HEIGHT_TILE
      to_y = to_y >= @height ? @height - 1 : to_y
    
      scroll_off_x = @scrolled_x % Common::TILE_SIZE
      scroll_off_y = @scrolled_y % Common::TILE_SIZE
    
      (from_y .. to_y).each do |y|
        (from_x .. to_x).each do |x|
          draw_x = (x - from_x) * Common::TILE_SIZE - scroll_off_x
          draw_y = (y - from_y) * Common::TILE_SIZE - scroll_off_y
        
          @tileset.at(@ground[y][x]).draw         draw_x, draw_y, Common::Z_GROUND
          @tileset.at(@animations[y][x][2]).draw  draw_x, draw_y, Common::Z_GROUND unless @animations[y][x].nil?
          @characters[y][x].draw                  @scrolled_x, @scrolled_y unless @characters[y][x].nil?
          draw_hotspot                            x, y, draw_x, draw_y if @wnd.show_debug
          @tileset.at(@layer[y][x]).draw          draw_x, draw_y, Common::Z_LAYER
        end
      end
    end
    ...
    def draw_caret
      c = 0x20FFFF80
      @wnd.draw_quad CARET_BOUNDS[0].first * Common::TILE_SIZE, CARET_BOUNDS[1].first * Common::TILE_SIZE, c,
        CARET_BOUNDS[0].last * Common::TILE_SIZE, CARET_BOUNDS[1].first * Common::TILE_SIZE, c,
        CARET_BOUNDS[0].first * Common::TILE_SIZE, CARET_BOUNDS[1].last * Common::TILE_SIZE, c,
        CARET_BOUNDS[0].last * Common::TILE_SIZE, CARET_BOUNDS[1].last * Common::TILE_SIZE, c,
        10000
    end
    ...
    def draw_hotspot(x, y, draw_x, draw_y)
      unless @hotspots[y][x].nil?
        ...
        @wnd.draw_quad draw_x, draw_y, c,
          draw_x + Common::TILE_SIZE, draw_y, c,
          draw_x, draw_y + Common::TILE_SIZE, c,
          draw_x + Common::TILE_SIZE, draw_y + Common::TILE_SIZE, c,
          10000
      end
    end
